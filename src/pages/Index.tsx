      ...
      <section id="login" className="relative py-16 sm:py-20 px-4">
        <div className="max-w-sm sm:max-w-md mx-auto">
          <div className="text-center mb-6">
            <h2 className="text-grad-brand text-lg tracking-[.18em] font-bold">QUEER SCENES</h2>
          </div>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fade} custom={0}>
            <div className="qs-card-lg p-6 sm:p-7">
              <h3 className="text-2xl font-bold text-[var(--t1)] mb-1">
                {isForgot ? "Reset password 🔑" : isSignUp ? "Create account 💜" : "Welcome back 👋"}
              </h3>
              <p className="text-[var(--t2)] text-sm mb-6">
                {isForgot ? "Enter your email to receive a reset link" : isSignUp ? "Join to start watching" : "Sign in to continue watching"}
              </p>
              <form onSubmit={handleLogin} className="space-y-4">
                {showNameFields && !isForgot && (
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      <label className="text-[11px] font-bold tracking-wider text-[var(--t2)]">FIRST NAME</label>
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[11px] font-bold tracking-wider text-[var(--t2)]">LAST NAME</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold tracking-wider text-[var(--t2)]">EMAIL</label>
                  <Input type="email" placeholder="your@email.com" value={email} onChange={(e) => setEmail(e.target.value)} className="qs-input" required />
                </div>
                {!isForgot && (
                  <div className="space-y-1.5">
                    <label className="text-[11px] font-bold tracking-wider text-[var(--t2)]">PASSWORD</label>
                    <div className="relative">
                      ...
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--t2)] hover:text-[var(--t1)] transition-colors">
                        {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                      </button>
                    </div>
                  </div>
                )}
                <Button type="submit" disabled={loading} className="qs-btn-primary w-full h-12">
                ...
                {!isSignUp && !isForgot && (
                  <div className="flex items-center justify-between pt-1">
                    <label className="flex items-center gap-2 text-xs text-[var(--t2)] cursor-pointer select-none">
                      <input type="checkbox" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} className="w-4 h-4 rounded border-border bg-[var(--s2)] accent-[var(--brand-purple)] cursor-pointer" />
                      Remember me
                    </label>
          ...
          <p className="text-center text-sm text-[var(--t2)] mt-5">
            {isForgot ? (
              <button type="button" onClick={() => setIsForgot(false)} className="font-semibold underline" style={{ color: "#a855f7" }}>
                Back to Sign in
              </button>
            ) : isSignUp ? (
