CREATE OR REPLACE FUNCTION public.admin_set_profile_premium(
  _profile_id uuid,
  _is_premium boolean,
  _premium_plan text DEFAULT NULL,
  _premium_expires_at timestamp with time zone DEFAULT NULL
)
RETURNS TABLE(
  id uuid,
  user_id uuid,
...
REVOKE ALL ON FUNCTION public.admin_set_profile_premium(uuid, boolean, text, timestamp with time zone) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.admin_set_profile_premium(uuid, boolean, text, timestamp with time zone) FROM anon;
GRANT EXECUTE ON FUNCTION public.admin_set_profile_premium(uuid, boolean, text, timestamp with time zone) TO authenticated;